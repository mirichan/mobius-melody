...
function createTileCircle(qtype, radius) {
    ...
    var tiles = qlist
        .map((q, i) => {
            var angle = angleOffset + alpha * i,
                t = createNewTile(i, q, a, width);
            t.move(
                x_padding + R + R * Math.cos(angle),
                y_padding + R + R * Math.sin(angle));
            t.rotate(angle);
            return t;
        });
...
function longestBackwardBranch(type) {
    return track.analysis[type]
        .map((q, i) => q.neighbors
            .map(n => i - n.dest.which))
        .reduce((a, e) => a.concat(e))
        .reduce((a, e) => Math.max(a, e), 0) * 100 / track.analysis[type].length;
}
